...
exports.cancelOrder = exports.getOrderById = exports.getCustomerOrders = exports.createOrder = void 0;
...
const createOrder = async (req, res) => {
    try {
        ...
        if (meals.length !== items.length) {
            return (0, helpers_1.sendError)(res, 'Some meals are not available', 400);
        }
        // Verify all meals are from the same provider
        const providerIds = [...new Set(meals.map((m) => m.providerId))];
        if (providerIds.length !== 1) {
            return (0, helpers_1.sendError)(res, 'All items must be from the same provider', 400);
        }
        ...
        return (0, helpers_1.sendSuccess)(res, order, 'Order placed successfully', 201);
    }
    catch (error) {
        console.error('Create order error:', error);
        return (0, helpers_1.sendError)(res, 'Failed to place order', 500);
    }
};
...
const getOrderById = async (req, res) => {
    try {
        ...
        if (role === 'CUSTOMER' && order.customerId !== userId) {
            return (0, helpers_1.sendError)(res, 'Access denied', 403);
        }
...
const cancelOrder = async (req, res) => {
    try {
        ...
        if (order.status !== 'PENDING') {
            return (0, helpers_1.sendError)(res, 'Only pending orders can be cancelled', 400);
        }
        const updatedOrder = await prisma.order.update({
            where: { id },
            data: { status: 'CANCELLED' },
        });
        return (0, helpers_1.sendSuccess)(res, updatedOrder, 'Order cancelled successfully');
    }
    catch (error) {
        console.error('Cancel order error:', error);
        return (0, helpers_1.sendError)(res, 'Failed to cancel order', 500);
    }
};
